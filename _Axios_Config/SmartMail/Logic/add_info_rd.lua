...
-- Log key data
--------------------------------------------------------------------------------------
LOGGER:info("Start of add_info_rd.lua:  EVENT: " .. (EVENT_TYPE or "") .. (EVENT_REF or "") 
...
-- Send note to assignee if action not taken by Service Desk or assigned team
	
	{
	name = [[ "Send note to assignee" ]],
		condition = [[
		...
		activities = {
			{ "email", 
				...
				subject = [[ strEmailSubjectPrefix .. "Additional Information has been added to " .. TICKET_TYPE_FOR_EMAIL .. " $EVENT_TYPE_N_EXT #$EVENT_TYPE_U$$EVENT_REF" ]],
				body = [[ strPathToTemplates .. "\\add_info.html" ]]
			}
		},
		"stop",
	},
	...
	name = [[ "Send note to assigned team" ]],
		...
		activities = {
			{ "email", 
				template_parameters = [[ { ASSIGNED_TO_TEXT = ASS_SVD_N } ]],
