...
-- Log key data
--------------------------------------------------------------------------------------
LOGGER:info("Start of Reopen_req.lua: \nEVENT: " .. (EVENT_TYPE or "") .. (EVENT_REF or "") )

set_of_rules {
	{
	name = [[ "Send note to assignee if we have an email" ]],
		...
		activities = {
			{ "email", 
				...
				subject = [[ strEmailSubjectPrefix .. "Reopen Request for" .. TICKET_TYPE_FOR_EMAIL .. " #$EVENT_TYPE_U$EVENT_REF" ]],
				body = [[ strPathToTemplates .. "\\Reopen_req.html" ]]
			}
		},
		"stop",
	},
